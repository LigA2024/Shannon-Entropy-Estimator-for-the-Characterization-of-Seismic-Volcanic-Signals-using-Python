...
This software is a product of the improvements made in version 1.0. This new version includes improvements to the analysis and calculation module II, which are as follows:

a) In addition to the already included analysis windows of 10 minutes, 1 hour, and 24 hours, a 5-minute window has been added.
b) A mathematical calculation has been added to the Entropy Envelope. It consists of representing a new graph of the Entropy Envelope by smoothing extreme values ??using the Cosine calculation algorithm, using Hann windows.
c) A fourth calculation metric has been added to the three existing ones (Entropy, Kurtosis, and Frequency Index). The fourth metric is Energy.

With these four metrics, the calculations are performed:
a) By selecting the analysis window size (5 and 10 minutes, 1 and 24 hours).
b) Selection of the Envelope parameters (Size, smoothing, and NaN tolerance)
c) Selection of the Start and End Time Intervals (by entering Julian days).
d) Selection of the start time to establish the values on the x-axis (using date:Year,month,day-YYY,MM,DD).
Selection of the filter type to use (LowPass, HighPass, BandPass, and BandStop).

The creation of the (.npy) file has been removed due to compatibility issues with different versions of Python libraries.
The creation of the (.CSV) file is retained to store the metadata of the metric results in a format compatible with spreadsheet software.
Future versions may reintroduce the creation of the (.npy) file if necessary (this file is optional).
...
The "Shannon Entropy Estimator for the Characterization of Seismic-Volcanic Signals using Python." is a user-friendly interface that 
...
1.        Performs signal reading and filtering actions, using one or three components, and provides graphical representations of both the original and filtered signals.

2.        Calculates and graphically represents metrics such as: Shannon Entropy (original and normalized), the envelope of the Shannon Entropy 
          and its smoothed version using a cosine function with a Hann window, Kurtosis, Frequency Index, and Energy, along with their respective envelopes.

3.       Presents a comparison of the Entropy envelope through the calculation of five types of frequency intervals within a filter previously selected by the user.
...
2) EntropySis1 folder
   ...
   h) "EntropySis1.py" Program of the interface. This interface contains the process of reading, filtering, calculating and graphically representing 
       the seismic-volcanic records of Shannon Entropy, Kurtosis, Frequency Index and Energy with their respective envelopes.
       Several analysis window sizes are used (5 and 10 minutes, one hour and 24 hours) and the user can also configure various window sizes when calculating the envelope. 
...
a)  Copy the (all) folder containing the Python and data files.
     This is according to the "English" version 1.2
...
---------------------------------------------------------------------------------------------------------
                 ...
                 =      Welcome to:                                           =
                 =                                                            =
                 =   Shannon Entropy Estimator for the Characterization of    =
                 =           Seismic-Volcanic Signals using Python.           =
                 =                        (Vers. 1.2)                         =
...
Soon, in a new version or update of the current version (1.2), extra algoruthms may be added, containing several types of analysis, functionalities or more methods, 
